...
import { getMenuDishCategoryById } from 'apiSdk/menu-dish-categories';
import { MenuDishCategoryInterface } from 'interfaces/menu-dish-category';

function MenuDishCategoryViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Menu Dish Category Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                <Box>
                  ...
                  >
                    Menu Dish Category Details
                  </Text>
                </Box>
                {hasAccess('menu_dish_category', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/menu-dish-categories/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('dish_category', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Dish Category"
                    text={
                      <Link as={NextLink} href={`/dish-categories/view/${data?.dish_category?.id}`}>
                        {data?.dish_category?.name}
                      </Link>
                    }
                  />
                )}
