...
function DashboardPage() {
  ...
  const dishCategoryStat = useSWR<PaginatedInterface<DishCategoryInterface>>(
    () => `/dish-categories`,
    () => getDishCategories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: dishCategoryStat?.data?.totalCount,
      isLoading: dishCategoryStat.isLoading,
