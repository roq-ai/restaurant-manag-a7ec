...
import { getDishCategoryMenuItemById, updateDishCategoryMenuItemById } from 'apiSdk/dish-category-menu-items';
...
function DishCategoryMenuItemEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<DishCategoryMenuItemInterface>(
    () => (id ? `/dish-category-menu-items/${id}` : null),
    () => getDishCategoryMenuItemById(id),
  );
  ...
  const handleSubmit = async (values: DishCategoryMenuItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateDishCategoryMenuItemById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Dish Category Menu Item',
              isCurrent: true,
            },
          ]}
        />
      ...
      <Box rounded="md">
        <Box mb={4}>
          <Text as="h1" fontSize={{ base: '1.5rem', md: '1.875rem' }} fontWeight="bold" color="base.content">
            Update Dish Category Menu Item
          </Text>
        </Box>
        {(error || formError) && (
          <Box mb={4}>
